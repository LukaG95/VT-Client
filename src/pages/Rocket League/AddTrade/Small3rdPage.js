...
function Small3rdPage({setShowPage, clickedItem}) {
  ...
  const {have, want, setHave, setWant, deleteRLitem} = useContext(TradeContextRL)
  ...
  useEffect(()=> {
    ...
    return (
      <div id="add-trade-3rd-page">
        ...
        <div className="add-trade-3rd-page-header-field">
          ...
          <div>
            <p>{itemName}</p>
            <button onClick={()=> {deleteRLitem(id); setShowPage("1")}}>Delete item</button>
          </div>
        </div>

        <FilterButton label="Color"         value={color}           setFunction={setColor}         dd={colorDD}          translate="translateY(-150px)" />
        <FilterButton label="Certification" value={certification}   setFunction={setCertification} dd={certDD}           translate="translateY(-215px)" />
        <FilterButton label="Amount"        value={amount}          setFunction={setAmount}        itemID={itemID}/>

        <button className="add-trade-confirm-button" onClick={()=> {submitFilters(); setShowPage("1")}}>CONFIRM</button>
    ...
    function submitFilters(){
      ...
      infoRL.Colors.map(info_color => {
        if (info_color.Name === color)
          colorID = info_color.ID
      })
  
      if (amount === "" || amount == 0) // this is so that if users set amount to zero or an empty string it will set it back to 1 when accepting filters
...
function FilterButton({dd, label, value, setFunction, itemID, translate}){
  ...
  function handleAmountSubmit(e){
    ...
    if ((isNaN(value) && value !== "") || value > (itemID === 4743 ? 100000 : 100) || value < 0 || value.length > 8)
      return
  ...
  if (label !== "Amount"){
    return(
      <div id={`${label}`} className="rl-icon-dropdown-button-section" style={open ? {zIndex: "2", marginBottom: "15px"} : {marginBottom: "15px"}}>
          ...
          onClick={()=> {
            ...
            if(!open){
             ...
             setTimeout(()=> {setOpen(!open)}, 500)
              ...
              document.getElementById(`${label}`).style.transform = "translateY(0px)"
              setOpen(!open)
            }
            
          }}

          style={open ? {border: "1px solid black", borderBottom: "none"} : null}
        ...
        {open && <DropdownMenu setFunction={setFunction} dd={dd} setOpen={setOpen} label={label}/>}
    ...
    return(
      <div className="rl-icon-amount-filter-field">
        ...
        <label> {label} - max {itemID === 4743 ? 100000 : 100}</label>
