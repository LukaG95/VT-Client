...
function App() { 
 ...
 const {isOpen_LeftSidebar, setIsOpen_LeftSidebar} = useContext(TbFiltersRLContext)
  ...
  return (
    <>
      ...
      <div className="shader_wrapper" id="main" >
        ...
        <div id="shader" onClick={() => {closeSidebar(); setIsOpen_LeftSidebar(false)}}></div>
        ...
        <div style={{position: "absolute", top: "25px", color: "white"}}>{height}</div>
          ...
          <Switch>
            ...
            <Route exact path="/">                  <> <FilterBar /><RLTrading /></>    </Route>
            <Route exact path="/trading/rl">        <> <FilterBar /><RLTrading /> </>    </Route>
            ...
            <Route exact path="/account/messages">  {handleRedirectOnRefresh(<></>)}    </Route>
            <Route exact path="/account/premium">   {handleRedirectOnRefresh(<Premium /> )}  </Route>
      ...
      <div onClick={()=> {window.scrollTo(0, 0)}} className="to-top">
        <img height={30} width={30} src={require('./images/other/arrow_nav_up.png')}/>
...
/*-----Functions                -------------*/
  ...
  function manageSidebarOnResize(){
    if (width > 1213 && isOpen_LeftSidebar) { // && sidebar is opened (for resizing performance)
      closeSidebar() 
     ...
     else if (width <=650 && isOpen_LeftSidebar){ //  && document.getElementById("main").style.transform !== "translateX(100%)"
      const main = document.getElementById("main")
      const sidebar = document.getElementById("sidebar")
