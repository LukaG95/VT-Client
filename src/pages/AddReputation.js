...
function AddReputation() {
  ...
  useEffect(()=> {
    ...
    axios.get(`/api/reputation/${userID}`)
    .then (res => { 
    ...
    <main className="repWrapper">
      ...
      <textarea 
        onChange={e => {
          ...
          if (repErrorMessage !== "You can't rep yourself" && repErrorMessage !== "You have to pick a rep category 1st")
            setRepErrorMessage("")
        }} 
  ...
  function handleRepSubmit(good_bad){
    ...
    if (repCategory === undefined){
      ...
      createNotification("error", "Pick a rep category 1st", "pick category 1st")
      return
    }

    if (feedback === undefined || feedback.replace(/\s/g, '').length < 5) {
      setRepErrorMessage("Your message has to be at least 5 characters long")
      createNotification("error", "Your message has to be at least 5 characters long", "5 char long message")
      return
    }
    ...
    if (!feedback.match(/^[\!@#$%^&*()\\[\]{}\-_+=~`|:;"'<>,./?a-zA-Z0-9 ]{5,100}$/gm)) {
      ...
      createNotification("error", "Your message includes inappropriate characters", "innapropriate characters")
      return
    }
    if (feedback.match(/\b(?:http:\/\/www\.|https:\/\/www\.|http:\/\/|https:\/\/)?[a-z0-9]+(?:[\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(?::[0-9]{1,5})?(?:\/.*)?\b/gm)) {
      ...
      createNotification("error", "Your message must not include links", "must not include links")
      return
    }
    ...
    .then(res => {console.log(res)
      if (res.data.info === "success"){
        ...
        setRepCategory()
        setRepErrorMessage("")
        createNotification("success", "Reputation was submitted", "rep submited")
      }
      else if (res.data.info === "hours24"){
        setRepErrorMessage("You can only leave a rep for the same person once in 24 hours")
        createNotification("error", `You have already repped ${repInfo.username}`, "already repped")
        return
      }
    })
