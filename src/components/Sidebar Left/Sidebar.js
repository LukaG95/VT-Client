...
function Sidebar({setIsOpen_LeftSidebar}){
  ...
  const {game, setGame, searchType, setSearchType, name, setName, color, setColor, cert, setCert, itemType, setItemType, platform, setPlatform, setTradeInfo, setPageAmount, resetFilters} = useContext(TbFiltersRLContext)
  ...
  return (
    <div id="sidebar">
      ...
      <div id="sidebar-1st-page">
        ...
        <Switch>
          <Route exact path="/">                    <RLBody />          </Route>
          <Route exact path="/trading/rl">          <RLBody />          </Route>
          <Route path="/reputation/add">            <SidebarBodyREP />         </Route>
          <Route path="/reputation">                <SidebarBodyREP />         </Route>
      ...
      <div id="sidebar-2nd-page">
        <div 
          className="sidebar-header-2nd-page" 
          onClick={goTo1stPage} 
        >
          <span id="back-arrow"> &lt; </span>

          <div className="header-selection-2nd-page">
            {selectedFilter}
          </div>
    ...
    return(
      <>  
        <div 
          ...
          onMouseEnter={()=> document.getElementById(`underline ${id}`).style.width="100%"} 
          onMouseLeave={()=> document.getElementById(`underline ${id}`).style.width="0px"} 
          onClick={() => {
            ...
            goTo2ndPage()
            setTimeout(() => document.getElementById('2nd-page-dropdown').focus(), 300) // should most likely be changed to something better (although it takes 250ms to load 2nd page, theoretically it should always work)
          }}
        >
          <div>
            <label>{text}</label>
        ...
        <div id={`underline ${id}`} className="side-filter-button-underline"></div>
      </>
    )
  }
  ...
  function goTo1stPage(){
    document.getElementById("sidebar-2nd-page").style.transform = "translateX(100%)"
    document.getElementById("sidebar-1st-page").style.transform = "translateX(0%)"
  }

  function goTo2ndPage(){
    document.getElementById("sidebar-1st-page").style.transform = "translateX(-100%)"
    document.getElementById("sidebar-2nd-page").style.transform = "translateX(0%)"
  }
