...
function Small1stPage({handleTradeSubmit, setShowPage, setClickedItem}) {
  ...
  const {have, want, platform, setPlatform, notes, setNotes, manageFocus, pushItem, clearWantItems, clearHaveItems, gotInfo, tradesAmount, setTradesAmount} = useContext(TradeContextRL)
  ...
  const displayed_have_items = have.map(item => {
    if (item.isAdded === false){
      if (item.isFocused === false) return <button name={item.id} onClick={e => {manageFocus(e); setShowPage("2")}}></button>
      else return <button name={item.id} onClick={e => {manageFocus(e); setShowPage("2")}} id="focusedButton">+</button>
    } 
    else return <AddedIconRL item={item} setShowPage={setShowPage} setClickedItem={setClickedItem}/>
  })
