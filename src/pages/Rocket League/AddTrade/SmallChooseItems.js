...
function SmallChooseItems({setShowPage, displayPage}) {
  ...
  const {have, want, clearHaveItems, clearWantItems} = useContext(TradeContextRL)
  ...
  useEffect(()=> {
      ...
      setItemImages(
        infoRL.Slots.map(Slot => Slot.Items.map(item => {
          if(item.Tradable) return(
            <LazyLoad scrollContainer={".item-imagesRL-SMALL"} placeholder={<Placeholder item={item}/>}>
              <div className="RLicon noUserInteraction" style={{minHeight: `${customHeight}px`}}>
      ...
      <div id="add-trade-2nd-page">
        ...
        <div className="rlChooseItemsSection-SMALL">
          ...
          <div className="item-imagesRL-SMALL">
            ...
            {itemImages ? itemImages : <p style={{color: "#f6f6f6", fontWeight: "800", margin: "10px 0px 0px 10px"}}>loading...</p>}
      ...
      if (have_count > 0 || want_count > 0)
        return(
          <div className="added-items-notice-phone">
            <div className="have-count-notice" style={focused_field !== "have" ? {opacity: "0.65"} : null}>
              <pre>Have:   {have_count}/12</pre>
              <img src={require(`../../../images/other/trash.png`)} style={{height: "20px", width: "20px", cursor: "pointer"}} onClick={()=> clearHaveItems()}/>
              <span></span>
            </div>

            <div className="want-count-notice" style={focused_field !== "want" ? {opacity: "0.65"} : null}>
              <pre>Want:   {want_count}/12</pre>
              <img src={require(`../../../images/other/trash.png`)} style={{height: "20px", width: "20px", cursor: "pointer"}} onClick={()=> clearWantItems()}/>
              <span></span>
            </div>
          </div>
        )
    ...
    function calculateHeight(){
      ...
      if (width <= 1213 && width >= 1101)
        return(105.2 - (1213-width)*0.1)
      ...
      else if (width <= 880 && width >= 771)
        return(104 - (880-width)*0.135)

      else if (width <= 770 && width >= 651)
        return(103.5 - (770-width)*0.1665)

      else if (width <= 650 && width >= 611)
        return(93.5 - (650-width)*0.17)

      else if (width <= 610 && width >= 501)
        return(104.8 - (610-width)*0.2)

      else if (width <= 500)
        return(140 - (500-width)*0.333)
